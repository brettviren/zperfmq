...
class zperfmq_t(Structure):
    pass # Empty - only for type checking
zperfmq_p = POINTER(zperfmq_t)

class int_t(Structure):
    pass # Empty - only for type checking
int_p = POINTER(int_t)
...
# zperfmq
lib.zperfmq_new.restype = zperfmq_p
lib.zperfmq_new.argtypes = []
lib.zperfmq_destroy.restype = None
lib.zperfmq_destroy.argtypes = [POINTER(zperfmq_p)]
lib.zperfmq_server.restype = int_p
lib.zperfmq_server.argtypes = [zperfmq_p, c_char_p]
lib.zperfmq_client.restype = int_p
lib.zperfmq_client.argtypes = [zperfmq_p, c_char_p]
lib.zperfmq_publish.restype = c_char_p
lib.zperfmq_publish.argtypes = [zperfmq_p, c_char_p]
lib.zperfmq_peers.restype = czmq.zhash_p
lib.zperfmq_peers.argtypes = [zperfmq_p]
lib.zperfmq_peer_info.restype = czmq.zhash_p
lib.zperfmq_peer_info.argtypes = [zperfmq_p, c_char_p]
lib.zperfmq_test.restype = None
lib.zperfmq_test.argtypes = [c_bool]

class Zperfmq(object):
    """
    API to a zperf agent which combines client, server and Zyre actor.
    """
    ...
    def __init__(self, *args):
        ...
        """
        if len(args) == 2 and type(args[0]) is c_void_p and isinstance(args[1], bool):
            self._as_parameter_ = cast(args[0], zperfmq_p) # Conversion from raw type to binding
            self.allow_destruct = args[1] # This is a 'fresh' value, owned by us
        elif len(args) == 2 and type(args[0]) is zperfmq_p and isinstance(args[1], bool):
            self._as_parameter_ = args[0] # Conversion from raw type to binding
            self.allow_destruct = args[1] # This is a 'fresh' value, owned by us
        else:
            assert(len(args) == 0)
            self._as_parameter_ = lib.zperfmq_new() # Creation of new raw type
            self.allow_destruct = True
    ...
    def __del__(self):
        ...
        """
        if self.allow_destruct:
            lib.zperfmq_destroy(byref(self._as_parameter_))
    ...
    def server(self, server_name):
        ...
        """
        return lib.zperfmq_server(self._as_parameter_, server_name)

    def client(self, client_name):
        """
        Create a client, return status code.
        """
        return lib.zperfmq_client(self._as_parameter_, client_name)

    def publish(self, nickname):
        """
...
Return our zyre peer id.
        """
        return lib.zperfmq_publish(self._as_parameter_, nickname)

    def peers(self):
        """
        Return a hash of zyre peer IDs and nick names.
        """
        return czmq.Zhash(lib.zperfmq_peers(self._as_parameter_), False)

    def peer_info(self, peerid):
        """
        Return information about a zyre peer.
        """
        return czmq.Zhash(lib.zperfmq_peer_info(self._as_parameter_, peerid), False)
    ...
    def test(verbose):
        ...
        Self test of this class.
        """
        return lib.zperfmq_test(verbose)
