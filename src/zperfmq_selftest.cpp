/*  =========================================================================
    zperfmq_selftest.c - run selftests
...
{
    test_item_t *item;
    printf ("Running zperfmq selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq (item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef ZPERFMQ_BUILD_DRAFT_API // selftest is still in draft
        else
            zperfmq_private_selftest (verbose, item->subtest);
#endif // ZPERFMQ_BUILD_DRAFT_API
    }
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("zperfmq_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running zperfmq test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef ZPERFMQ_BUILD_DRAFT_API // selftest is still in draft
        else
            zperfmq_private_selftest (verbose, test->subtest);
#endif // ZPERFMQ_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
